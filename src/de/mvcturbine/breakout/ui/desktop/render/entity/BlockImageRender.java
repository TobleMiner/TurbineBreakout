...
public class BlockImageRender extends EntityRender
{
	...
	public BlockImageRender()
	{
		try
		{
			textures[0] = ImageIO.read(this.getClass().getResourceAsStream(
					"/de/mvcturbine/breakout/textures/blockYellow.png"));
			textures[1] = ImageIO.read(this.getClass().getResourceAsStream(
					"/de/mvcturbine/breakout/textures/blockGreen.png"));
			textures[2] = ImageIO.read(this.getClass().getResourceAsStream(
					"/de/mvcturbine/breakout/textures/blockRed.png"));
			textures[3] = ImageIO.read(this.getClass().getResourceAsStream(
					"/de/mvcturbine/breakout/textures/blockBlue.png"));
			// textures[4] = ImageIO
	...
	public void renderEntity(Entity e, BufferedImage img, Size2D scale)
	{
		...
		if(scaled.get(imgSize) == null)
		{
			...
			{
				Image displayedImage = textures[i].getScaledInstance((int) width,
						(int) height, BufferedImage.SCALE_FAST);
		...
		gfx.drawImage(scaled.get(imgSize)[block.getDurability() - 1],
				(int) (block.getLocation().getX() * scale.getX()),
				img.getHeight() - (int) ((block.getSize().getHeight() +
						block.getLocation().getY()) * scale.getY()),
				null);
